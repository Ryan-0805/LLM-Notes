...
GraphRAG核心思想：首先从源文档中派生一个实体知识图谱（通过大模型提取实体、关系，社区检测提取社区），然后为所有密切相关的实体组预先生成社区摘要。如果是全局搜索，汇总与用户问题有关的社区摘要最终生成一个全局性的答案；如果是局部搜索，会检索问题中的实体，找到实体相关的边和实体以及实体所在的文本块，汇总并排序和过滤，最终生成答案。
